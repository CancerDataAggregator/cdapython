...
def fetch_rows(
    ...
    *,
    ...
    #############################################################################################################################
    # Ensure nothing untoward got passed into the `count_only` or `debug` parameters.

    if count_only != True and count_only != False:
        
        print( f"summary_counts(): ERROR: The `count_only` parameter must be set to True or False; you specified '{count_only}', which is neither.", file=sys.stderr )
    ...
    # `project` entity.
    # 
    # Provenance information encoded in "`table`_identifier" can be recovered
    # by using the separate identifiers() function.

    for column_to_remove in [ f"{table}_associated_project", f"{table}_identifier" ]:
    ...
    #############################################################################################################################
    # Manage basic validation for the `add_columns` parameter, which enumerates user-specified non-`table` columns to be
    # joined with the main `table` result data.

    # First: `link_to_table` is just a macro to fetch all the rows from a particular table.
    # Translate it to `add_columns` and process as normal (overwriting anything specified in
    # `add_columns` whenever `link_to_table` is nonempty -- see the docstring entry for
    # `link_to_table` for context).
    ...
    if link_to_table != '':
        ...
        if link_to_table not in tables():
    ...
    # Validate the contents of (the current, possibly updated version of) `add_columns`.
    ...
    # Save the name and data type of the ID field of the join table for any extra non-`table` columns, so we can present well-formed output later in a consistent way.
    ...
    parsed_add_columns = source_table_columns_in_order.copy()
    ...
    if join_table_id_field is not None:
        ...
        parsed_add_columns.append( join_table_id_field )
    ...
    for column_to_add in add_columns:
        ...
        if column_to_add not in parsed_add_columns:
            
            use_only_default_columns = False

            parsed_add_columns.append( column_to_add )

    query_for_add_columns = None

    if not use_only_default_columns:
        
        query_for_add_columns = Query()

        query_for_add_columns.node_type = 'SELECT'

        query_for_add_columns.l = Query()

        query_for_add_columns.l.node_type = 'SELECTVALUES'

        query_for_add_columns.l.value = ', '.join( parsed_add_columns )
    ...
    # If the user requested extra columns via the `add_columns` parameter,
    # set that up.
    # 
    ...
    if query_for_add_columns is not None:
        
        query_for_add_columns.r = final_query

        final_query = query_for_add_columns
    ...
    # is the same whether or not additional column data (from other tables) has been
    # requested.
    ...
    for column_name in result_dataframe:
        ...
        if column_name not in parsed_add_columns:
    ...
    for column in parsed_add_columns:
