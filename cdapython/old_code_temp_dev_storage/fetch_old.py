...
#############################################################################################################################
#
# fetch_rows_old( table=`table` ): Get CDA data records ('result rows') from `table` that match user-specified criteria.
#
#############################################################################################################################
...
def fetch_rows_old(
    ...
    *,
    ...
    if debug != True and debug != False:
        print(
            f"fetch_rows(): ERROR: The `debug` parameter must be set to True or False; you specified '{debug}', which is neither.",
            file=sys.stderr,
        )
    ...
    if link_to_table != "" and len(add_columns) > 0:
        ...
        if len(join_tables) > 1:
            print(
                f"fetch_rows(): ERROR: extra information from outside the '{table}' table, requested via the 'add_columns' parameter, must all come from a single table. You have specified columns ({add_columns}) from more than one extra table ({sorted( join_tables )}).",
                file=sys.stderr,
            )
    ...
    for item in match_all:
        ...
        if matching_column_count != 1:
            print(
                f"fetch_rows(): ERROR: match_all: requested column '{filter_column_name}' is not a searchable CDA column.",
                file=sys.stderr,
            )
        ...
        if filter_operator not in operators_by_data_type[target_data_type]:
            print(
                f"fetch_rows(): ERROR: match_all: operator '{filter_operator}' is not usable for values of type '{target_data_type}'.",
                file=sys.stderr,
            )
        ...
        if filter_value.lower() != "null":
            if target_data_type == "boolean":
                ...
                if filter_value not in boolean_alias:
                    print(
                        f"fetch_rows(): ERROR: match_all: requested column {filter_column_name} has data type 'boolean', requiring a true/false value; you specified '{filter_value}', which is neither.",
                        file=sys.stderr,
                    )
                ...
                if re.search(r"^[-+]?\d+(\.\d+)?$", filter_value) is None:
                    print(
                        f"fetch_rows(): ERROR: match_all: requested column {filter_column_name} has data type '{target_data_type}', requiring a number value; you specified '{filter_value}', which is not.",
                        file=sys.stderr,
                    )
                ...
                if re.search(r"\*", filter_value) is not None:
                    print(
                        f"fetch_rows(): ERROR: match_all: wildcards (*) are only allowed at the ends of string values; string '{original_filter_value}' is noncompliant (it has one in the middle). Please fix.",
                        file=sys.stderr,
                    )
                ...
                print(
                    f"fetch_rows(): ERROR: match_all: unanticipated `target_data_type` '{target_data_type}', cannot continue. Please report this event to CDA developers.",
                    file=sys.stderr,
                )
        ...
        if filter_value.lower() == "null":
            if filter_operator == "=":
                ...
                print(
                    f"fetch_rows(): ERROR: match_all: filter query '{filter_column_name} {filter_operator} {filter_value}' is malformed: please use only = and != when filtering on NULL.",
                    file=sys.stderr,
                )
    ...
    for item in match_any:
        ...
        if matching_column_count != 1:
            print(
                f"fetch_rows(): ERROR: match_any: requested column '{filter_column_name}' is not a searchable CDA column.",
                file=sys.stderr,
            )
        ...
        if filter_operator not in operators_by_data_type[target_data_type]:
            print(
                f"fetch_rows(): ERROR: match_any: operator '{filter_operator}' is not usable for values of type '{target_data_type}'.",
                file=sys.stderr,
            )
        ...
        if filter_value.lower() != "null":
            if target_data_type == "boolean":
                ...
                if filter_value not in boolean_alias:
                    print(
                        f"fetch_rows(): ERROR: match_any: requested column {filter_column_name} has data type 'boolean', requiring a true/false value; you specified '{filter_value}', which is neither.",
                        file=sys.stderr,
                    )
                ...
                if re.search(r"^[-+]?\d+(\.\d+)?$", filter_value) is None:
                    print(
                        f"fetch_rows(): ERROR: match_any: requested column {filter_column_name} has data type '{target_data_type}', requiring a number value; you specified '{filter_value}', which is not.",
                        file=sys.stderr,
                    )
                ...
                if re.search(r"\*", filter_value) is not None:
                    print(
                        f"fetch_rows(): ERROR: match_any: wildcards (*) are only allowed at the ends of string values; string '{original_filter_value}' is noncompliant (it has one in the middle). Please fix.",
                        file=sys.stderr,
                    )
                ...
                print(
                    f"fetch_rows(): ERROR: match_any: unanticipated `target_data_type` '{target_data_type}', cannot continue. Please report this event to CDA developers.",
                    file=sys.stderr,
                )
        ...
        if filter_value.lower() == "null":
            if filter_operator == "=":
                ...
                print(
                    f"fetch_rows(): ERROR: match_any: filter query '{filter_column_name} {filter_operator} {filter_value}' is malformed: please use only = and != when filtering on NULL.",
                    file=sys.stderr,
                )
    ...
    if url_override is not None and len(url_override) > 0:
        ...
        if debug:
            ...
            print("BEGIN DEBUG MESSAGE: fetch_rows(): Loaded CDA_API_URL from environment", file=sys.stderr)
            ...
            print("END  DEBUG MESSAGE: fetch_rows(): Loaded CDA_API_URL from environment", file=sys.stderr)
        ...
        if debug:
            ...
            print("BEGIN DEBUG MESSAGE: fetch_rows(): Loaded CDA API URL from default config", file=sys.stderr)
            ...
            print("END  DEBUG MESSAGE: fetch_rows(): Loaded CDA API URL from default config", file=sys.stderr)
    ...
    if debug:
        ...
        print(
            f"BEGIN DEBUG MESSAGE: fetch_rows(): Querying CDA API '{table}' endpoint ({fetch_message})", file=sys.stderr
        )
            ...
            if isinstance(paged_response_data_object, ApplyResult):
                ...
                try:
                    ...
                    if re.search("urllib3.exceptions.MaxRetryError", str(type(e))) is not None:
                        ...
                        print(
                            f"fetch_rows(): ERROR: Something ({type(e)}) went wrong when trying to connect to the API. Please check settings (rerunning the last call with debug=True will give more information).",
                            file=sys.stderr,
                        )
    ...
    if len(result_dataframe.columns) > 0:
        ...
        for column in columns_to_fetch:
            ...
            if result_column_data_types[column] in {"numeric", "integer", "bigint"}:
                ...
                print(
                    f"fetch_rows(): ERROR: Unexpected data type `{result_column_data_types[column]}` received; aborting. Please report this event to the CDA development team.",
                    file=sys.stderr,
                )
